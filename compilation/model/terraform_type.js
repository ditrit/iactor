...
export class TerraformType extends TerraformNode {
  ...
  setId(name, parsed_rule, category) {
    const current_st = parsed_rule.ctx.prog.current_service_template;
    const parent_st = parsed_rule.ctx.prog.current_parent_service_template;
    this.name = name;
    ...
    const namespace = (current_st.ns_uri) ? current_st.ns_uri : namespace_name;
    if (parent_st) {
