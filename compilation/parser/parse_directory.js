...
export function parse_directories(filesPath) {
  ...
  for (let i = 0; i < result.modules.length; i++) {
    ...
    if (path.includes('..')) result = parse_directory(path.replace('..', modulePath), result, result.modules[i].name);
    else if (path.includes('.')) result = parse_directory(path.replace('.', modulePath), result, result.modules[i].name);
    else result = parse_directory(modulePath + path, result, result.modules[i].name);
  }
