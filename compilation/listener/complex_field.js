...
export default {
    ...
    exit_complex_field(parsed_rule) {
        ...
        let name = (parsed_rule.ctx.ctx.getText()) ? parsed_rule.ctx.ctx.getText().split('{')[0] : ""
        let objects = (prog.complex_field_object != []) ? prog.complex_field_object : []
        parsed_rule.prog.current_file.complex_field.push(newTerraformComplexField({name: name, objects : objects}, parsed_rule.ctx))
