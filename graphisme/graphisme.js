...
const datas = get_datas("./testsTeraform/tf");
	...
	datas.forEach((blockEnd) => {
		if (blockEnd.link) {
			blockEnd.link.forEach((blockBegin) => {
				...
				let blockBeginWidth =
          blockBegin.width > 0 ? blockBegin.width + 30 : 160;
				let blockEndHeight = blockEnd.height > 0 ? blockEnd.height + 30 : 44;
				let blockBeginHeight =
          blockBegin.height > 0 ? blockBegin.height + 30 : 44;
